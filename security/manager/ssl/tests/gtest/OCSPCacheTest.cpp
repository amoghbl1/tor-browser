          ...
          PRErrorCode error, PRTime time)
{
  ...
  ASSERT_TRUE(time >= 10);
  SECStatus rv = cache.Put(subject, issuer, nullptr, error, time - 10, time);
  ASSERT_TRUE(rv == SECSuccess);
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_TRUE(cache.Get(subject, issuer, nullptr, errorOut, timeOut));
...
TEST_F(OCSPCacheTest, TestPutAndGet)
{
  ...
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_FALSE(cache.Get(&issuer, &issuer, nullptr, errorOut, timeOut));
}
...
TEST_F(OCSPCacheTest, TestVariousGets)
{
  ...
  ASSERT_FALSE(cache.Get(&subject, &issuer, nullptr, errorOut, timeOut));
}
...
TEST_F(OCSPCacheTest, TestEviction)
{
  ...
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_FALSE(cache.Get(&evictedSubject, &issuer, nullptr, errorOut, timeOut));
}
...
TEST_F(OCSPCacheTest, TestNoEvictionForRevokedResponses)
{
  ...
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_TRUE(cache.Get(&notEvictedSubject, &issuer, nullptr, errorOut, timeOut));
...
TEST_F(OCSPCacheTest, TestEverythingIsRevoked)
{
  ...
  SECStatus result = cache.Put(&goodSubject, &issuer, nullptr, 0, timeIn + 1025 - 50,
                               timeIn + 1025);
  ASSERT_TRUE(result == SECSuccess);
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_FALSE(cache.Get(&goodSubject, &issuer, nullptr, errorOut, timeOut));
  ...
  result = cache.Put(&revokedSubject, &issuer, nullptr, SEC_ERROR_REVOKED_CERTIFICATE,
...
TEST_F(OCSPCacheTest, VariousIssuers)
{
  ...
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_TRUE(cache.Get(&subject, &issuer1, nullptr, errorOut, timeOut));
  ASSERT_TRUE(errorOut == 0 && timeOut == timeIn);
  ASSERT_FALSE(cache.Get(&subject, &issuer2, nullptr, errorOut, timeOut));
  ASSERT_FALSE(cache.Get(&subject, &issuer3, nullptr, errorOut, timeOut));
}
...
TEST_F(OCSPCacheTest, Times)
{
  ...
  SECStatus rv = cache.Put(&subject, &issuer, nullptr, SEC_ERROR_OCSP_UNKNOWN_CERT, 100, 100);
  ASSERT_TRUE(rv == SECSuccess);
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_TRUE(cache.Get(&subject, &issuer, nullptr, errorOut, timeOut));
...
TEST_F(OCSPCacheTest, NetworkFailure)
{
  ...
  SECStatus rv = cache.Put(&subject, &issuer, nullptr, PR_CONNECT_REFUSED_ERROR, 300, 350);
  ASSERT_TRUE(rv == SECSuccess);
  PRErrorCode errorOut;
  PRTime timeOut;
  ASSERT_TRUE(cache.Get(&subject, &issuer, nullptr, errorOut, timeOut));
  ...
  // This should not override the already present entry.
  rv = cache.Put(&subject, &issuer, nullptr, PR_CONNECT_REFUSED_ERROR, 500, 550);
  ASSERT_TRUE(rv == SECSuccess);
  ASSERT_TRUE(cache.Get(&subject, &issuer, nullptr, errorOut, timeOut));
  ...
  // This should not override the already present entry.
  rv = cache.Put(&subject, &issuer, nullptr, PR_CONNECT_REFUSED_ERROR, 700, 750);
  ASSERT_TRUE(rv == SECSuccess);
  ASSERT_TRUE(cache.Get(&subject, &issuer, nullptr, errorOut, timeOut));
